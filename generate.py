...
def generate(model, ir_path, vis_path, text_path, result, mode):
    result_path = "results"
    # ir_img = utils.get_test_images_vis(ir_path, mode=mode)
    # vis_img = utils.get_test_images_vis(vis_path, mode=mode)
    ...
    predicted_classes = probabilities.float()
...
#
#     # 对张量进行填充，以确保它具有所需的长度
#     padded_text = torch.nn.functional.pad(text, (0, 0, 0, max_length - text.size(0))) if text.size(
#         0) < max_length else text
#
#     # 将填充后的文本特征张量移动到GPU（如果尚未移动）
#     padded_text = padded_text.cuda()
#
...
#     ir_filename_without_ext = os.path.splitext(ir_filename)[0]  # 移除文件扩展名
#
#     img_fusion= _generate_fusion_image(model, ir_img, vis_img, padded_text)
#
#     #probabilities = torch.sigmoid(classes)
#
#     # # 设置阈值，这里以0.5为例
